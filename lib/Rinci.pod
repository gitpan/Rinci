...
=head2 Version 1.1.30 (2012-09-07)
...
=head2 Version 1.1.29 (2012-09-04)

=over 4

=item *

Transaction: In 'check_state' phase, specify that result message should contain a description of what needs to be fixed, or how it is already fixed, or how it is unfixable; the message can then be logged by TM to be displayed to user.

=back

=head2 Version 1.1.28 (2012-08-29)
...
=head2 Version 1.1.27 (2012-08-28)
...
=head2 Version 1.1.26 (2012-08-22)
...
=head2 Version 1.1.25 (2012-08-22)
...
=head2 Version 1.1.24 (2012-08-21)

=over 4

=item *

[ENHANCEMENTS] Specify TM's interface in more details (moved from Riap::Transaction, which now becomes shorter).

=item *

[ENHANCEMENTS] Specify steps for action, rollback, and crash recovery in more details, with example.
...
=head2 Version 1.1.22 (2012-08-09)

=over 4

=item *

tx: Describe the ordering of calls during undo/rollback/redo when there is nested call. The same function may be called twice or more with parts of undo/redo steps.

=item *

tx: In transaction mode, function now gets undo/redo data from the usual -undo_data special argument, like in non-transaction mode. Txm's API get_undo_steps() and get_redo_steps() are now removed.

=back

=head2 Version 1.1.21 (2012-07-23)

=over 4

=item *

[INCOMPATIBLE CHANGES] function: Remove argument specification key 'src', use 'cmdline_src' instead. I think 'src' is too general.

=back

=head2 Version 1.1.20 (2012-07-21)
...
=head2 Version 1.1.19 (2012-06-22)
...
=head2 Version 1.1.18 (2012-06-06)
...
=head2 Version 1.1.17 (2012-05-31)
...
=head2 Version 1.1.16 (2012-05-03)
...
=head2 Version 1.1.15 (2012-05-02)
...
[INCOMPATIBLE CHANGE] Update Sah schema syntax (pre-0.03, [merge:X] -> [mergeX])

=back

=head2 Version 1.1.14 (2012-05-02)

=over 4

=item *

[INCOMPATIBLE CHANGE] Change 'exec' dependency clause to 'prog' (avoid possible confusion because 'exec' can imply that we need to execute the program; there can be other future dep clause for that).

=back

=head2 Version 1.1.13 (2012-03-23)

=over 4

=item *

[ENHANCEMENTS] Each tag in 'tags' property can also be a tag metadata hash (for

=item *

[ENHANCEMENTS] translatable message, etc).

=back

=head2 Version 1.1.12 (2012-03-13)

=over 4

=item *

[ENHANCEMENTS] Add 'Rinci::result'.

=back

=head2 Version 1.1.11 (2012-03-13)

=over 4

=item *

[ENHANCEMENTS] Add property 'entity_version'.
...
[INCOMPATIBLE CHANGES] package: Remove property 'pkg_version' (use 'entity_version'

=item *

[INCOMPATIBLE CHANGES] instead).

=back

=head2 Version 1.1.10 (2012-02-28)
...
=head2 Version 1.1.9 (2012-02-28)

=over 4

=item *

[INCOMPATIBLE CHANGE] function: Change (back) 'set' alias spec to 'code', in arg spec

=item *

[INCOMPATIBLE CHANGE] 'aliases' (for backward compatibility with 1.0)

=back

=head2 Version 1.1.8 (2012-02-28)

=over 4

=item *

[INCOMPATIBLE CHANGES] function: Replace (back) 'alias_for' argument spec with

=item *

[INCOMPATIBLE CHANGES] 'cmdline_aliases'. I first used 'alias_for' so I can list each alias as

=item *

[INCOMPATIBLE CHANGES] a key in 'args' property. The goal is so I can get all argument names

=item *

[INCOMPATIBLE CHANGES] aliases) simply by doing a keys() on 'args' hash. And the goal of

=item *

[INCOMPATIBLE CHANGES] that is to add a 'complete_arg_name' Riap action which is more

=item *

[INCOMPATIBLE CHANGES] lightweight than a full 'meta' just to get argument names. Turns out

=item *

[INCOMPATIBLE CHANGES] that I don't need 'alias_for' just to support 'complete_arg_name', and

=item *

[INCOMPATIBLE CHANGES] also turns out that completing argument name needs more than just all

=item *

[INCOMPATIBLE CHANGES] argument names + aliases. You also need 'pos' information. So a full

=item *

[INCOMPATIBLE CHANGES] 'meta' is currently used. The reason I now revert to 'cmdline_aliases'

=item *

[INCOMPATIBLE CHANGES] style is because I don't want command-line aliases to become

=item *

[INCOMPATIBLE CHANGES] full/first-class argument.

=back

=head2 Version 1.1.7 (2012-02-23)
...
=head2 Version 1.1.6 (2012-02-21)
...
=head2 Version 1.1.5 (2012-02-10)
...
Declare that 1.1 series might introduce minor backward compatibility problems between revisions.

=item *

Rename 'arg_pass_style' and 'result_envelope' to the old (Sub::Spec-era) 'args_as' and 'result_naked'. New names are not better.

=item *

Code entity URI now moved to Riap specification as the 'riap' URI scheme.
...
Change syntax of code entity URI, from 'Pkg.SubPkg.func' to 'pm:/Pkg/SubPkg/func' (or 'py:', 'php:', and so on).
...
=head2 Version 1.1.2 (2012-01-19)

=over 4

=item *

Add 'default_lang' property. Add guidelines on what to put in 'summary' and 'description'.
...
=head2 Version 1.1.1 (2012-01-18)
...
function: Incompatible changes to 'args' and 'result' properties; 'args' is now a hash of arg names and arg *specs* (instead of arg schemas). 'result' is now a hash of data, instead of schema. The purpose is to keep Sah schema clean from custom, non-schema-related schema clauses, like arg_* (thus requiring custom Sah extensions, etc). Mixing them into schemas was not the right way.

=item *

function: property 'type' removed, replaced with 'is_func', 'is_meth', 'is_static_meth'. This is because a single subroutine/function can act as all.

=item *

function: Other properties which are also removed/replaced: 'timeout' (non-core), 'retry' (non-core), 'scope' (can be replaced by using tags), 'result_naked' (replaced by 'result_envelope'), 'args_as' (replaced by 'arg_pass_style').

=item *

function: property 'deps': terminology change ('dep clause' -> 'dep type'), rename dep types: 'sub' -> 'func', 'mod' -> 'perl_module'.
